...
describe('/tournament endpoint', () => {
  ...
  describe('[POST] when creating a participants', () => {
    ...
    it('le participant a été ajouté au tournois', async () => {
      ...
      const participant = await request(app).post(`/api/tournaments/${tournament.body.id}/participants`).send(exampleParticipant).expect(200);

      await request(app).delete(`/api/tournaments/${tournament.body.id}/participants`).expect(200);
      await request(app).delete(`/api/participants/${participant.body.id}`).expect(200);
    });

    it('le tournoi n\'existe pas', async () => {
      await request(app).post('/api/tournaments/123/participants').expect(404);
    });
  ...
  describe('[GET] tournament participants', () => {
    ...
    it('liste des participants au tournois', async () => {
      ...
      expect(get.body.name).toEqual(exampleParticipant.name);
